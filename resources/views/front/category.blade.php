...
<body class="font-[Poppins] pb-[83px]">
	...
	<section id="Category-result" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px]">
		...
		<div id="search-cards" class="grid grid-cols-3 gap-[30px]">
			@forelse ($category->news as $articles)
                <a href="{{ route('front.detail', $articles->slug) }}" class="card">
                    <div class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
                        <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                            <div
                                ...
                                <p class="text-xs leading-[18px] font-bold uppercase">{{ $articles->category->name }}</p>
                            </div>
                            <img src="{{ Storage::url($articles->thumbnail) }}" alt="thumbnail photo"
                                class="w-full h-full object-cover" />
                        </div>
                        <div class="flex flex-col gap-[6px]">
                            <h3 class="text-lg leading-[27px] font-bold">{{ substr($articles->name, 0, 50)  }} {{ strlen($articles->name) > 50 ? "..." : "" }}</h3>
                            <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $articles->created_at->format('d M Y') }}</p>
                        </div>
                    </div>
                </a>
            @empty
                <p class="text-bold">Tidak ada artikel terbaru</p>
            @endforelse
